class H264Decoder {
    constructor(type, canvasElement, statusElement, timeElement, fpsElement, framesElement, errorElement) {
    ...
    async initialize() {
        try {
            ...
            const config = {
                ...
                hardwareAcceleration: this.type === 'hardware' ? 'prefer-hardware' : 'prefer-software',
            ...
            if (!support.supported) {
                throw new Error(`${this.type === 'hardware' ? '硬件' : '软件'}解码配置不支持`);
            }
        ...
        for (let i = 0; i < frameInfo.frames.length; i++) {
            ...
            try {
                const chunk = new EncodedVideoChunk({
                    ...
                    data: frameData,
                    duration: i < frameInfo.frames.length - 1 ? 
                        (frameInfo.frames[i + 1].timestamp - frame.timestamp) * 1000 / 90 : 66
                });

                this.decoder.decode(chunk);
                console.log(`解码帧 ${i}: ${frame.isKeyFrame ? 'Key' : 'Delta'} frame, 大小: ${frameSize}, 时间戳: ${frame.timestamp/90}`);
    ...
    handleFrame(frame) {
        ...
        console.log(`${this.type} 解码器收到第 ${this.frameCount} 帧，时间戳: ${frame.timestamp}`);
    }
    ...
    renderLoop() {
        ...
        // 如果找到了应该渲染的帧，并且不是当前帧
        if (frameToRender && frameIndex >= 0 && frameIndex !== this.currentFrameIndex) {
...
class H264TestApp {
    ...
    initializeEventListeners() {
        ...
        fileTypeRadios.forEach(radio => {
            radio.addEventListener('change', (e) => this.handleFileTypeChange(e.target.value));
        });

        // 文件选择
        this.h264FileInput.addEventListener('change', (e) => this.handleH264FileSelect(e));
        this.binFileInput.addEventListener('change', (e) => this.handleBinFileSelect(e));
        ...
        if (this.syncPrevBtn) {
            this.syncPrevBtn.addEventListener('click', () => this.syncPreviousFrame());
        }
        if (this.syncNextBtn) {
    ...
    updateMediaDumpInfo() {
        ...
        if (jsonFile && this.frameInfo) {
            ...
            info += `MID: ${this.frameInfo.mid || 'N/A'}`;
        }
    ...
    async startTest() {
        ...
        if (this.currentFileType === 'mediaDump' && !this.frameInfo) {
            this.globalError.textContent = '请选择完整的 MediaDump 文件（.bin 和 .json）';
            return;
        }
    ...
    enableSyncControls() {
        if (this.softwareDecoder.frameQueue.length > 0 && this.hardwareDecoder.frameQueue.length > 0) {
    ...
    updateSyncFrameInfo() {
        ...
        const totalFrames = Math.max(this.softwareDecoder.frameQueue.length, this.hardwareDecoder.frameQueue.length);
